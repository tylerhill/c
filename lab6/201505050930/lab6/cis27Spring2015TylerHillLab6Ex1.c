...
ExprTermAddr createExpressionTermTH(void);
void displayETerm(ExprTermAddr);
void displayENode(ExprNodeAddr);
ExprNodeAddr createExpressionNodeTH(ExprTermAddr);
void insertExpressionNodeTH(ExprListAddr, ExprNodeAddr);
void displayENodeList(ExprList);
void printList(PListTH);
void printInFix(ExprList);
...
ExprTermAddr createExpressionTermTH() {
	char userOp;
	ExprTermAddr eTerm;
...
void insertExpressionNodeTH(ExprListAddr eList, ExprNodeAddr eNode) {
	if (*eList == 0) {
		*eList = eNode;
	} else {
		eNode->next = *eList;
...
void displayENodeList(ExprList eList) {
	if (eList != 0) {
		if (eList->next != 0) {
			displayENode(eList);
			eList = eList->next;
...
void printInFix(ExprList eList) {
	while (eList != 0) {
		printf("%c",eList->eTermAddr->op);
		displayFrTylerHill(&(eList->eTermAddr->termAddr->coE));
		printf("^%d",eList->eTermAddr->termAddr->ex);
		eList = eList->next;
	}
}
...
void menu() {
	...
	do {
		...
		scanf("%d", &option);
			...
			switch (option) {
			case 1:
				insertExpressionNodeTH(&list,createExpressionNodeTH(createExpressionTermTH()));	
				break;
			case 2:
				printInFix(list);
				break;
			case 3:
				displayENodeList(list);
				break;
			case 4:
